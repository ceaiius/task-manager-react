  ...
  return (
    ...
    >
        <div className="mb-2 sm:mb-0 flex-grow mr-4">
            <span className={`text-gray-800 dark:text-gray-100 ${isCompleted && !isDeleting ? 'line-through text-gray-500 dark:text-gray-400' : ''} ${isProcessing ? 'opacity-70' : ''}`}>
                {task.title}
            </span>
            {formattedDueDate && (
                <p className={`text-xs mt-1 ${dateColorClass} ${isProcessing ? 'opacity-70' : ''}`}>
      ...
      <div className="flex items-center space-x-2 flex-shrink-0">
          <button
              ...
              className={`p-1 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-1 dark:focus:ring-offset-gray-800 transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed
                  ${isToggling ? 'text-yellow-600' : ''}
                  ${isCompleted
                      ? 'text-yellow-500 hover:bg-yellow-100 dark:hover:bg-yellow-900/30 hover:text-yellow-700 focus:ring-yellow-500'
                      : 'text-green-500 hover:bg-green-100 dark:hover:bg-green-900/30 hover:text-green-700 focus:ring-green-500'
                  }
               `}
          ...
          <button
              ...
              disabled={isProcessing}
              className={`p-1 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-1 dark:focus:ring-offset-gray-800 transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed
                  ${isDeleting ? 'text-red-600' : 'text-gray-400 hover:text-red-600 hover:bg-red-100 dark:hover:bg-red-900/30 focus:ring-red-500'}
              `}
